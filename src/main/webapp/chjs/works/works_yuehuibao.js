...
var TableInit =function () {
	...
	var address=addurl+"/works/yuehuibao_query";
...
//			showRefresh: true,                  // 是否显示刷新按钮
			...
			uniqueId: "workyuehuibaoid",                     // 每一行的唯一标识，一般为主键列
...
//					search: params.search, // 工具栏查询内容,search:true才有
			...
			columns : [ {
				...
				formatter: function(value,row,index){
                    return '<a href="javascript:void(0);" onclick="yuehuibao_look('+row.workyuehuibaoid+')" >查看</a>';//zhuohuibao_look('+row+')
				}
...
function table_search(){
	$('#table_data').bootstrapTable('refresh', {url: addurl+"/works/yuehuibao_query"});
}
...
function add_data(){
	...
	$.ajax({
		type:"POST",
		url:addurl+'/works/yuehuibao_add',
		async:false,
		cache:true,
...
function del_data(){
	...
	swal({
    	...
    	var workyuehuibaoids=new Array();
    	for(var i=0;i<IdSelections.length;i++){
    		workyuehuibaoids.push(IdSelections[i].workyuehuibaoid);
    	}
    	
    	parent.onloading();
    	$.ajax({
    		type:"POST",
    		url:addurl+'/works/yuehuibao_del',
    		async:false,
    		cache:true,
    		data:{workyuehuibaoids:workyuehuibaoids},
    		success: function(result){
    			parent.removeload();
    			if(result=='ok'){
    				swal("" ,result,"success");
...
function edit_dialog(){
	...
	$('#dialog_form #workyuehuibaoid').val(IdSelections[0].workyuehuibaoid);
...
function yuehuibao_look(workyuehuibao){
	var rowdata=$('#table_data').bootstrapTable('getRowByUniqueId', workyuehuibao);
	var userid=$('#userid').val();
	var level=$('#level').val();
