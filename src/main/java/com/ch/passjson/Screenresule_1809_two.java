...
public class Screenresule_1809_two {
						...
						if(kstate==0){
							...
							if(moduleidstr==Integer.parseInt(ScreenResult.get("ModuleID").toString())||moduleidstr==-1){
								...
								for(int o=0;o<ScreenResult.size();o++){
									...
									if(!"ScreenElements".equals(name)){
										if(ScreenResult.get(name).equals(ScreenResult1.get(name))){
											sum1=sum1+1;
										}
									}else{
										int numinit=0;
										JSONObject ScreenElements=ScreenResult.getJSONObject("ScreenElements");
										JSONObject ScreenElements1=ScreenResult1.getJSONObject("ScreenElements");
										JSONArray OrderIndexs=ScreenElements.getJSONArray("OrderIndexs");
										JSONArray OrderIndexs1=ScreenElements1.getJSONArray("OrderIndexs");
										if(!OrderIndexs.toString().equals(OrderIndexs1.toString())){
											for(int i=0;i<OrderIndexs.size();i++){
												String num=OrderIndexs.getString(i);
												String num1=OrderIndexs1.getString(i);
												if(num==num1){
													OrderIndexs.remove(i);
													OrderIndexs1.remove(i);
												}
											}
											if(!OrderIndexs.toString().equals(OrderIndexs1.toString())){
												numinit=1;
										...
										if(!ScreenElements.get("PassRule").equals(ScreenElements.get("PassRule"))){
											numinit=1;
										}
										if(numinit==0){
											sum1=sum1+1;
...
//									System.out.println(ScreenResult1.toString());
					...
					if(sum<(ScreenResult.names().size()+3)&&sum>0){
						...
						String errorstrs="";
						String errorstrs1="";
						for(int i=0;i<errnamelist.size();i++){
							errorstrs=errorstrs+errobj.get(errnamelist.get(i))+",";
							errorstrs1=errorstrs1+errobj1.get(errnamelist.get(i))+",";
						}
						listerr.add("DrugIndex=====>"+drugindex+",ScreenResults :"+errnamelist1);
						listerr.add("DrugIndex=====>"+drugindex+",ScreenResults-error :"+errnamelist+"<br>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp=========Show Errors=========");
						listerr.add("----error:"+errorstrs1);
						listerr.add("----error:"+errorstrs);
					}
